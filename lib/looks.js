export function newLook(name) {
  return { id: crypto.randomUUID(), name, createdAt: new Date().toISOString(), items: [], renders: [] };
}

export function addItem(look, product, variant) {
  if (look.items.some((i) => i.variantId === variant.id && i.storeOrigin === product.storeOrigin)) return look;
  return {
    ...look,
    items: [...look.items, {
